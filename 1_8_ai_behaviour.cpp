
///////////
//////////////
///////////////// AI behaviour
//////////////
///////////

///// start war

#define AI_START_WAR_IF_NEIGHBOUR_HAS_LESS_EQUIPMENT_THRESHOLD 0.2

#define AI_START_WAR_IF_NEIGHBOUR_HAS_MORE_FACTORIES_THRESHOLD 4.0

#define AI_START_WAR_FOR_EQUIPMENT_FACTORY_RATIO_THRESHOLD 1.2

#define AI_DONT_START_WAR_CHANCE_IF_NOT_FULL_STRENGTH 0.9

///// end war

#define AI_STOP_WAR_IF_WE_HAVE_LESS_EQUIPMENT_THRESHOLD 0.2

#define AI_STOP_WAR_IF_PRODUCTION_FOR_A_COUPLE_OF_DAYS_EXCEEDS_EQUIPMENT 10 // if the current amount of equipment that we have could be produced in less than current amount of time stop fighting

///// special behaviour

#define AI_ENGAGE_GANGBANG_AT_RATIO 0.7 // if there is one MF that is too powerfull the whole world is going to unite against him
